...
class TimeoutChecker implements TransitionCheckerInterface
{
    use LoggerTrait;
    ...
    public function check(): int
    {
        ...
        foreach ($this->workflowRepository->getDatabaseStateMachineNames() as $stateMachineName) {
            $affectedItems += $this->checkStateMachineTimeouts($stateMachineName);
        }
     ...
     */
    protected function checkStateMachineTimeouts(string $stateMachineName): int
    {
        try {
            return $this->stateMachineFacade->checkTimeouts($stateMachineName);
        } catch (Throwable $throwable) {
            $this->getLogger()->error(
                sprintf('Timeout check skipped for state machine "%s": %s', $stateMachineName, $throwable->getMessage()),
                ['exception' => $throwable],
            );
